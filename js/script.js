...
// ========== RENDER MOBIL (TANPA TOMBOL TELEPON) ==========
function renderCars() {
    ...
    carsGrid.innerHTML = cars
        .map((car) => {
            ...
            // Tombol WhatsApp berubah jika sudah dirental
            let waButtonHTML;
            if (isRented) {
                waButtonHTML = `
          <button class="btn-wa btn-rented" disabled style="background:#555; color:#aaa; cursor:not-allowed; opacity:0.7; width:100%;">
            <i class="fas fa-times-circle"></i> Sudah Dirental
          </button>
        `;
            } else {
                waButtonHTML = `
        ...
        <div class="car-card" data-car-id="${car.id}">
            ...
            <div class="car-info">
                ...
                <div class="car-actions" style="display:flex; gap:0;">
                    ${waButtonHTML}
                </div>
            </div>
        </div>
